...
export function buildFixedCupBracket({
  clubIds
}: BuildFixedCupBracketProps): CupBracket {
...
export async function persistCupMatches({
  db,
  competitionId,
  bracket
}: PersistCupMatchesProps) {
    ...
    for (const match of round.matches) {
      const [createdMatch] = await db
        ...
        .values({
          ...
          homeFromMatchId: match.homeFromMatchIndex !== undefined
            ? insertedMatchIdsByRound[roundIndex - 1][match.homeFromMatchIndex]
            : null,

          awayFromMatchId: match.awayFromMatchIndex !== undefined
