...
import {
  acceptTransferProposalForUser,
  denyTransferProposalForUser,
} from "@/services/transferProposalResolution";
...
export const protectedTransferProposalRoutes = async (fastify: FastifyInstance) => {
  ...
  fastify.post("/transfer-proposals/:proposalId/deny", async (request, reply) => {
    const paramsSchema = z.object({
      proposalId: z.uuid(),
    });
    ...
    try {
      const proposal = await denyTransferProposalForUser({
        userId: session.user.id,
        proposalId,
      });
      ...
      request.log.error(error, "Failed to deny transfer proposal");
      ...
      if (error instanceof Error) {
        ...
        if (error.message === "Only proposal target player can deny this transfer.") {
          return reply.status(403).send({ error: error.message });
        }

        if (
          error.message === "Transfer proposal is already resolved." ||
          error.message === "Unable to resolve transfer proposal as denied."
        ) {
          return reply.status(400).send({ error: error.message });
        }
      }
